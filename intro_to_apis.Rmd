...
**About**: This document demonstrates how to get data from a public API with R. In this example, data is retrieved from [Parliaments's API](https://developer.parliament.uk/) in JSON format using the [httr2 package](https://httr2.r-lib.org/).
...
```
In the response object above, a status code of 200 (OK) can in the {status_code} variable. The desired data exists as non-human readable raw bytes in the {body} variable. Before doing anything with this data, we must first convert it.
